...
void rounded_box_smooth(rgb *pixels, size_t width, size_t height, rgb bg, rgb fg)
{
    for (size_t y = 0; y < height; ++y)
    {
        for (size_t x = 0; x < width; ++x)
        {
            float d = sdf_rounded_box(x, y, width / 2.0f, height / 2.0f, width / 4.0f - ROUNDED_BOX_RADIUS,
                                      height / 4.0f - ROUNDED_BOX_RADIUS, ROUNDED_BOX_RADIUS);
            pixels[y * width + x] = smooth(d, bg, fg);
        }
    }
}
...
void shapes(rgb *pixels, size_t width, size_t height, rgb bg, rgb circle, rgb box, rgb rounded_box)
{
    ...
    for (size_t y = 0; y < height; ++y)
    {
        for (size_t x = 0; x < width; ++x)
        {
            ...
            case 0: { // circle
                float cx = gap + radius;
                pixels[y * width + x] = smooth(sdf_circle(x, y, cx, height / 2.0f, radius), bg, circle);
            }
            break;
            case 1: { // box
                float cx = gap + shape_diameter + gap + radius;
                pixels[y * width + x] = smooth(sdf_box(x, y, cx, height / 2.0f, radius, radius), bg, box);
            }
            break;
            case 2: { // rounded_box
                float cx = gap + shape_diameter + gap + shape_diameter + gap + radius;
                pixels[y * width + x] = smooth(sdf_rounded_box(x, y, cx, height / 2.0f, radius - ROUNDED_BOX_RADIUS,
                                                               radius - ROUNDED_BOX_RADIUS, ROUNDED_BOX_RADIUS),
                                               bg, rounded_box);
            }
            break;
            }
    ...
    for (size_t y = 0; y < height; ++y)
    {
        for (size_t x = 0; x < width; ++x)
        {
            ...
            case 2: { // subtraction
                ...
                float cx_b = gap + shape_diameter + gap + shape_diameter + gap + shape_diameter * 2 / 3;
...
void scene(rgb *pixels, size_t width, size_t height, float t)
{
    ...
    float stars[][3] = {
        {width * 0.08f, height * 0.07f, 2.0f}, {width * 0.84f, height * 0.11f, 1.5f},
        {width * 0.13f, height * 0.78f, 1.5f}, {width * 0.79f, height * 0.83f, 2.0f},
        {width * 0.93f, height * 0.44f, 1.5f}, {width * 0.04f, height * 0.38f, 1.0f},
        {width * 0.61f, height * 0.04f, 1.5f}, {width * 0.37f, height * 0.94f, 1.0f},
        {width * 0.71f, height * 0.93f, 1.0f}, {width * 0.96f, height * 0.76f, 1.5f},
    };
    ...
    for (size_t y = 0; y < height; ++y)
    {
        for (size_t x = 0; x < width; ++x)
        {
            ...
            for (size_t i = 0; i < sizeof(stars) / sizeof(stars[0]); ++i)
            {
                ...
                color = lerp_color(color, lerp_color(bg, p1_col, 0.12f), smoothstep(stars[i][2] * 4.0f, 0.0f, d));
                color = lerp_color(color, p1_col, smoothstep(1.0f, -1.0f, d));
            }

            color =
                lerp_color(color, orb_col, smoothstep(1.2f, 0.0f, fabsf(sdf_circle(px, py, cx, cy, orbit1))) * 0.8f);
            color =
                lerp_color(color, orb_col, smoothstep(1.2f, 0.0f, fabsf(sdf_circle(px, py, cx, cy, orbit2))) * 0.8f);
            color =
                lerp_color(color, orb_col, smoothstep(1.2f, 0.0f, fabsf(sdf_circle(px, py, cx, cy, orbit3))) * 0.8f);
            ...
            color = lerp_color(color, lerp_color(bg, p1_col, 0.18f), smoothstep(r1 * 2.5f, 0.0f, d_p1));
            color = lerp_color(color, p1_col, smoothstep(1.0f, -1.0f, d_p1));

            float d_p2 = sdf_circle(px, py, p2x, p2y, r2);
            color = lerp_color(color, lerp_color(bg, p2_col, 0.18f), smoothstep(r2 * 2.5f, 0.0f, d_p2));
            ...
            color = lerp_color(color, lerp_color(bg, p3_col, 0.18f), smoothstep(r3 * 2.5f, 0.0f, d_p3));
            color = lerp_color(color, p3_col, smoothstep(1.0f, -1.0f, d_ring) * 0.55f);
            color = lerp_color(color, p3_col, smoothstep(1.0f, -1.0f, d_p3));

            float d_sta_body = sdf_rounded_box(px, py, stax, stay, sta_bw - sta_br, sta_bw - sta_br, sta_br);
            float d_sta_panels = sdf_box(px, py, stax, stay, sta_pw, sta_ph);
            float d_station = sdf_union(d_sta_body, d_sta_panels);
            color = lerp_color(color, lerp_color(bg, sta_col, 0.20f), smoothstep(sta_bw * 3.0f, 0.0f, d_station));
